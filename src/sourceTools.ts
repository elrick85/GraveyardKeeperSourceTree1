...
const findAndMapByKey = (key: string) => {
  const res = [];
  SOURCE_TOOLS.forEach((tool) => {
    let sourceItems = tool.items.filter((v) => v.key.toLocaleLowerCase() === key.toLocaleLowerCase());
    sourceItems = sourceItems.map((v) => {
      const items = v.items.map((item) => ({...item, img: findImgByKey(item.key), wiki: findWikiLinkByKey(item.key)}));
      return {...v, tool: tool.key, items};
    });
    res.push(...sourceItems);
  });
...
const findImgByKey = (key): string => {
  const record = imgsData.find((v) => v.key === key);

  return record ? record.link : '';
};

const findWikiLinkByKey = (key): string => {
  const record = imgsData.find((v) => v.key === key);

  return record ? record.wiki : '';
};

const sortOutTree = (tree: SourceItem[]) => {
  const result = tree.map((v) => {
    const vItems = v.items.map((foo) => {
      const fooItems = foo.items.map((bar) => {
        let items = findAndMapByKey(bar.key);
        let tools;

        if (items) {
          items = items.map((item, i) => {
            return {...item, key: `Option ${i + 1}`};
          });

          tools = [...new Set(items.map((item) => item.tool))];
        }

        return {...bar, items, tools, img: findImgByKey(bar.key), wiki: findWikiLinkByKey(bar.key)};
      });

      return {...foo, items: fooItems, img: findImgByKey(foo.key), wiki: findWikiLinkByKey(foo.key)};
    });

    return {...v, items: vItems, img: findImgByKey(v.key), wiki: findWikiLinkByKey(v.key)};
  });

  return result;
};

export const SOURCE_TOOLS_TREE = sortOutTree(SOURCE_TOOLS);

console.log(SOURCE_TOOLS_TREE);
